...
const getUsersInfo = async () => {
    ...
    const rocketchat_getUserListAll_response = await rocketchat.get(
        "/api/v1/users.list",
        {
            headers: {
    ...
    const users = _.filter(
        rocketchat_getUserListAll_response.data.users,
        (user) => {
            const lastLogin = new Date(user.lastLogin);

            return (
                lastLogin < dateFns.subDays(today, DAYS) ||
                (user.status === "online" &&
                    lastLogin < dateFns.subDays(today, DAYS))
            );
        }
    ...
    users.forEach(
        ({ _id, username, name, lastLogin, createdAt, roles, status }) => {
